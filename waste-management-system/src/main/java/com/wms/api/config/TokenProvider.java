...
public class TokenProvider implements Serializable {
	...
	public String generateToken(Authentication authentication) {
		...
		return Jwts.builder().setSubject(authentication.getName())
				.claim(SecurityError.AUTHORITIES_KEY.getValue(), authorities)
				.signWith(SignatureAlgorithm.HS256, SecurityError.SIGNING_KEY.getValue())
				.setIssuedAt(new Date(System.currentTimeMillis())).setExpiration(new Date(System.currentTimeMillis()
						+ SecurityError.ACCESS_TOKEN_VALIDITY_SECONDS.getValueInLong() * 1000))
				.compact();
	}
	...
	UsernamePasswordAuthenticationToken getAuthentication(final String token, final Authentication existingAuth,
		...
		final JwtParser jwtParser = Jwts.parser().setSigningKey(SecurityError.SIGNING_KEY.getValue());
		...
		final Collection<? extends GrantedAuthority> authorities = Arrays
				.stream(claims.get(SecurityError.AUTHORITIES_KEY.getValue()).toString().split(","))
